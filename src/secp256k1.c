...
int secp256k1_ec_pubkey_decompress(const secp256k1_context_t* ctx, unsigned char *pubkey, int *pubkeylen) {
    return secp256k1_ec_pubkey_decompress_copy(ctx, pubkey, pubkey, pubkeylen);
}

int secp256k1_ec_pubkey_decompress_copy(const secp256k1_context_t* ctx, const unsigned char *pubkeyin, unsigned char *pubkeyout, int *pubkeylen) {
    secp256k1_ge_t p;
    int ret = 0;
